...
class import_invoice_wizard(models.TransientModel):
    ...
    _inherit='import.invoice.wizard'
        ...
        if self.product_details_option == 'from_product':
            ...
            if account_inv_brw.type == "out_invoice" and account_inv_brw.state == 'draft':
                ...
                if cust_account_id:
                    ...
                    print(tec_str)
                    print(store_str)
                    select_queru = """INSERT INTO account_move_line """ + tec_str + """ VALUES """ + store_str
                                

                    account_move_line = self.env.cr.execute(select_queru,val_tuple)
                        



                    
                    inv_lines = self.env['account.move.line'].search([],order='id desc', limit=1)
