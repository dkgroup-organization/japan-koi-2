...
class SaleShop(models.Model):
    ...
    get_order_from = fields.Datetime('Date From')
    get_order_to = fields.Datetime('Date To')
                ...
                for attrs in attributes:
                    ...
                    attrs_op_values = {
                        ...
                        "name": self.get_value_data(
                            self.get_value(product_options.get('name').get('language'))) + ' [' + self.get_value_data(
                            product_options.get('id')) + ']',
                    }
        ...
        try:
            ...
            if self.get_value_data(customer_detail.get('customer').get('passwd')):
                customer_ids = res_partner_obj.search(
                    [('presta_id', '=', self.get_value_data(customer_detail.get('customer').get('id'))[0]),
                     ('customer_rank', '=', True), ('supplier_rank', '=', False), ('manufacturer', '=', False)])
                if not customer_ids:
    ...
    def get_value(self, data):
        ...
        if isinstance(data, list):
            ...
            lang_id = self.env['prestashop.language'].search(
                [('code', '=', 'fr'), ('presta_instance_id', '=', self.prestashop_instance_id.id)])
            if not lang_id:
                lang = self.env['prestashop.language'].search([])
                lang_id = self.env['prestashop.language'].search(
    ...
    def create_address(self, address_dict, prestashop, address_type):
        try:
            address_id = False
            res_partner_obj = self.env['res.partner']
            ...
            country_id = int(address_dict.get('id_country'))
            state_id = int(address_dict.get('id_state'))
            ...
            if address_dict.get('id_customer') != '0':
                parent_id = res_partner_obj.search([('presta_id', '=', address_dict.get('id_customer'))], limit=1)
                if not parent_id:
                    try:
                ...
                parent_id = res_partner_obj.search([('presta_id', '=', address_dict.get('id_supplier'))], limit=1)
                if not parent_id:
                    try:
                ...
                parent_id = res_partner_obj.search([('presta_id', '=', address_dict.get('id_manufacturer'))], limit=1)
                if not parent_id:
                    try:
            ...
            address_id = res_partner_obj.search([('address_id', '=', address_dict.get('id'))])
            if address_id:
    ...
    def create_presta_product(self, product, prestashop):
        ...
        try:
            ...
            # TODO uncomment this code if we merge product from odoo with prestashop in the future
            ...
            if last_import_product:
                ...
                if product.get('products') and product.get('products').get('product'):
                    for attrs in product.get('products').get('product'):
                        ...
                        if data:
                            # if data == str(6330):
                            print("import_products id", data)
                            # product1 = self.env['product.template'].search([('presta_id','=',data)])
                            # if not product1:
    ...
    def manageOrderLines(self, orderid, order_detail, prestashop):
        ...
        for child in order_rows:
            ...
            # Get Tax
            more_order_details = prestashop.get('order_details', self.get_value_data(child.get('id')))
            if more_order_details and more_order_details.get('order_detail'):
                vat_id = more_order_details.get('order_detail').get('associations').get('taxes').get('tax')
                if vat_id and vat_id.get('id'):
                    try:
                        vat_rate = prestashop.get('taxes', vat_id.get('id'))
                        if vat_rate and vat_rate.get('tax'):
                            vat_rate = vat_rate.get('tax').get('rate')
                            vat_rate = float(vat_rate)
                            if vat_rate:
                                tax_odoo = self.env['account.tax'].sudo().search(
                                    [('type_tax_use', '=', 'sale'), ('amount', '=', vat_rate)], limit=1)
                                if tax_odoo:
                                    tax_id = tax_odoo.id
                                    line.update(
                                        {'price_unit': str(self.get_value_data(child.get('unit_price_tax_excl')))})
                                    line.update({'tax_id': [(4, tax_odoo.id)]})
                    except:
                        pass
            if self.get_value_data(child.get('product_attribute_id')) != '0':
                ...
                try:
                    ...
                    if temp_ids:
                        ...
                        'combination_id', '=', self.get_value_data(child.get('product_attribute_id')))])
                        if not product_ids:
                            product_ids = product_obj.search([('product_tmpl_id', '=', temp_ids[0].id)])
                        new_product_ids = 0
                        for product_id in product_ids:
                            ...
                            if product_id.product_template_attribute_value_ids:
                                for product_template_attribute_value_id in product_id.product_template_attribute_value_ids:
                                    ...
                                    if product_template_attribute_value_id.name:
                                        product_attribute_name = product_template_attribute_value_id.name
                                        product_attribute_name = re.sub("[\[].*?[\]]", "",
                                                                        product_attribute_name).rstrip()
                                    value_list_attribute_name = ""
                                    if value_list:
                                        value_list_attributes = prod_attr_val_obj.browse(value_list)
                                        for value_list_attribute in value_list_attributes:
                                            value_list_attribute_name = re.sub("[\[].*?[\]]", "",
                                                                               value_list_attribute.name).rstrip()
                                            if product_attribute_name == value_list_attribute_name and product_id.product_tmpl_id == \
                                                    temp_ids[0]:
                                                count_ok += 1
                            if count_ok and count_ok == len(product_id.product_template_attribute_value_ids):
                        ...
                        if new_product_ids:
                            ...
                            removed_product += 1
                            p_ids = product_obj.search([('default_code', '=', 'RMVPRD' + str(removed_product))])
                            if not p_ids:
                                pid = product_obj.create(
                                    {'name': 'Removed Product', 'default_code': 'RMVPRD' + str(removed_product)})
                            else:
                                pid = p_ids[0]
        ...
        discoun = float(self.get_value_data(order_detail.get('total_discounts_tax_incl')))
        if discoun and discoun > 0:
            discount_product = self.env['product.product'].search([('name', '=', 'Réduction')])
            dline = {
                'product_id': discount_product.id,
                'product_uom': discount_product.uom_id.id,
                'price_unit': (-1 * discoun),
                'product_uom_qty': 1,
                'order_id': orderid.id,
                'tax_id': False,
            }
            if tax_id:
                dline.update(
                    {'price_unit': (-1 * float(self.get_value_data(order_detail.get('total_discounts_tax_excl'))))})
                dline.update({'tax_id': [(4, tax_id)]})
            dline_ids = sale_order_line_obj.search(
                [('product_id', '=', self.get_value_data(dline.get('product_id'))), ('order_id', '=', orderid.id)])
            if not dline_ids:
                sale_order_line_obj.create(dline)
            else:
                dline_ids[0].write({'price_unit': dline_ids[0].price_unit - discoun})
        ...
        if ship and ship > 0:
            ...
            vat_rate = self.get_value_data(order_detail.get('carrier_tax_rate'))
            if vat_rate:
                vat_rate = float(vat_rate)
                if vat_rate:
                    tax_odoo = self.env['account.tax'].sudo().search(
                    ...
                    if tax_odoo:
                        sline.update(
                            {'price_unit': float(self.get_value_data(order_detail.get('total_shipping_tax_excl')))})
                        sline.update({'tax_id': [(4, tax_odoo.id)]})
        ...
        wrapping = float(self.get_value_data(order_detail.get('total_wrapping_tax_incl', 0)))
        if wrapping and wrapping > 0:
            ...
            if tax_id:
                dline.update({'price_unit': float(self.get_value_data(order_detail.get('total_wrapping_tax_excl')))})
                dline.update({'tax_id': [(4, tax_id)]})
    ...
    def create_presta_order(self, order_detail, prestashop):
        ...
        try:
            ...
            if partner_ids:
                ...
                if self.get_value_data(order_detail.get('id_customer')) == '0':
                    customer = res_partner_obj.search([('name', '=', 'Guest')])
                    ...
                    try:
                        ...
                        cust_id = self.create_customer(cust_data, prestashop)
                        if cust_id:
                            customer = cust_id
                        else:
            ...
            id_address_delivery = res_partner_obj.search([('presta_id', '=', order_detail.get('id_address_delivery'))],
                                                         limit=1)
            if not id_address_delivery:
                try:
                    address_data = prestashop.get('addresses', order_detail.get('id_address_delivery'))
                    id_address_delivery = self.create_address(address_data.get('address'), prestashop, 'delivery')
                    if id_address_delivery:
                        order_vals.update({'partner_shipping_id': id_address_delivery.id})
                except Exception as e:
                    pass
            id_address_invoice = res_partner_obj.search([('presta_id', '=', order_detail.get('id_address_invoice'))],
                                                        limit=1)
            if not id_address_invoice:
                try:
                    address_inv_data = prestashop.get('addresses', order_detail.get('id_address_invoice'))
                    id_address_invoice = self.create_address(address_inv_data.get('address'), prestashop, 'invoice')
                    if id_address_invoice:
                        order_vals.update({'partner_invoice_id': id_address_invoice.id})
                except Exception as e:
                    pass
            ...
            if not round(s_id.amount_total, 2) == float(self.get_value_data(order_detail.get('total_paid_tax_incl'))):
                real_amount = s_id.amount_total
                s_id.write({
                    'original_odoo_total_amount': real_amount,
                    'amount_untaxed': float(self.get_value_data(order_detail.get('total_paid_tax_excl'))),
                    'amount_total': float(self.get_value_data(order_detail.get('total_paid_tax_incl'))),
                })
    ...
    def import_orders(self):
        ...
        for shop in self:
            prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,
                                                  shop.prestashop_instance_id.webservice_key or None)
            ctx = {}
            if shop.get_order_from:
                start_datetime = shop.get_order_from
            else:
                start_datetime = datetime.now()
                start_datetime = start_datetime.replace(year=2021, month=5, day=1, hour=0, minute=0, second=0)
            if shop.get_order_to:
                end_datetime = shop.get_order_to
            else:
                end_datetime = start_datetime + relativedelta(days=31)
            ...
            order_data = prestashop.get('orders', options={
                'filter[invoice_date]': '[' + str_start_datetime + ',' + str_end_datetime + ']', 'date': '1'})
            ...
            if order_data.get('orders') and order_data.get('orders').get('order'):
                ...
                for order in orders:
                    ...
                    print("import_order id", order_id)
                    sale_order = self.env['sale.order'].search([('presta_id', '=', order_id)])
                    if not sale_order:
