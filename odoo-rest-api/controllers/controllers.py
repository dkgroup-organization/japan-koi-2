...
class OdooAPI(http.Controller):
    ...
    @http.route('/create_contact', type='json', auth='user', methods=['POST'], csrf=False)
    def create_contact(self, **post):
        model_to_post = request.env['res.partner']
        data = False
        try:
            data = post['data']
            res = model_to_post.create(data)
            return res
        except KeyError:
            msg = "`data` parameter is not found on POST request body"
    ...
    @http.route('/get_all_variants', auth="user", methods=['GET'], csrf=False)
    def get_all_variants(self, **post):
        variant_dict = {}
        try:
            model_to_get = request.env['product.product']
            variant_ids = model_to_get.sudo().search([])
            if variant_ids:
                for rec in variant_ids:
                    variant_dict[rec.id] = {
                                            ...
                                            'product_categ_id':rec.categ_id.name,
