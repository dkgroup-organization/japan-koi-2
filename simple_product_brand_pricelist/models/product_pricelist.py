...
class ProductPricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        ...
        product_li = []
        for rec in products:
            product_li.append(rec.id)
        product_li_obj = self.env['product.product'].browse(product_li)
        brands = product_li_obj.mapped('simple_product_brand_id')
        ...
        self._cr.execute(
            ...
            'ON item.simple_product_brand_id = brand.id '
            'WHERE (item.product_tmpl_id IS NULL OR item.product_tmpl_id = any(%s))'
            'AND (item.product_id IS NULL OR item.product_id = any(%s))'
            'AND (item.categ_id IS NULL OR item.categ_id = any(%s)) '
            'AND (item.simple_product_brand_id IS NULL OR item.simple_product_brand_id = any(%s)) '
        ...
        for product, qty, partner in products_qty_partner:
            ...
            for rule in items:
                ...
                if rule.simple_product_brand_id:
                    if product.simple_product_brand_id != rule.simple_product_brand_id:
                        continue
